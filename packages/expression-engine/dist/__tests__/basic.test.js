...
describe('ExpressionEngine - 基于 jexl', () => {
    ...
    describe('jexl 高级特性', () => {
        it('应该支持条件表达式', async () => {
            const result = await defaultEngine.process('用户类型: {{state.user.vip ? "VIP会员" : "普通用户"}}', context);
            expect(result).toBe('用户类型: VIP会员');
        });
        it('应该支持数组操作', async () => {
            const result = await defaultEngine.process('商品数量: {{state.items | length}}', context);
            expect(result).toBe('商品数量: 3');
        });
        it('应该支持数组过滤', async () => {
            const result = await defaultEngine.process('未读消息: {{state.notifications[.read == false] | length}}', context);
            expect(result).toBe('未读消息: 2');
        });
        it('应该支持字符串转换', async () => {
            const result = await defaultEngine.process('用户名: {{state.user.name | upper}}', context);
            expect(result).toBe('用户名: 张三');
        });
        it('应该支持数学运算', async () => {
            const result = await defaultEngine.process('明年年龄: {{state.user.age + 1}}', context);
            expect(result).toBe('明年年龄: 26');
        });
        it('应该支持比较运算', async () => {
            const result = await defaultEngine.process('是否成年: {{state.user.age >= 18 ? "是" : "否"}}', context);
    ...
    describe('自定义转换函数', () => {
        it('应该能添加自定义转换函数', async () => {
            ...
            // 添加格式化货币的转换函数
            engine.addTransform('currency', (value) => {
